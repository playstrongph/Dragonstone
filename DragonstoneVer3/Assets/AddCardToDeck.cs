...
public class AddCardToDeck : MonoBehaviour
{
    ...
    public void SetCardAsset(HeroAsset asset) { HeroAsset = asset; } 

/*
    void OnMouseDown()
    {
        HeroAsset asset = GetComponent<AddCardToDeck>().HeroAsset;
        if (asset == null)
            return;

        // check that these cards are available in collection (Quantity>0) or (TotalQuantity-AmountAlreadyInDeck)>0
        if (CardCollection.Instance.QuantityOfEachCard[HeroAsset] - DeckBuildingScreen.Instance.BuilderScript.NumberOfThisCardInDeck(HeroAsset) > 0)
        {
    ...
    void OnRightClick()
    {
        ...
        if (Physics.Raycast(clickPoint, out hitPoint))
        {
            ...
            if (hitPoint.collider == this.GetComponent<Collider>())
            {
                ...
                CraftingScreen.Instance.ShowCraftingScreen(GetComponent<AddCardToDeck>().HeroAsset);
            }
        }
    }
*/
    ...
    {
        int quantity = CardCollection.Instance.QuantityOfEachCard[HeroAsset];
...
//            quantity -= DeckBuildingScreen.Instance.BuilderScript.NumberOfThisCardInDeck(HeroAsset);
