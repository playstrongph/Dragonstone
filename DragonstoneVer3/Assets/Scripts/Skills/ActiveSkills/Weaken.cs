...
public class Weaken : SkillComponent
{
    ...
    {                  
       ...
       tree.AddCurrent(InflictAttackDownCoroutine(Attacker, Target));
       tree.AddCurrent(InflictSlowCoroutine(Attacker, Target));
       tree.AddCurrent(InflictAntiBuffCoroutine(Attacker, Target));
    ...
    IEnumerator InflictSlowCoroutine(GameObject Attacker, GameObject Target)
    {
    ...
    IEnumerator InflictAttackDownCoroutine(GameObject Attacker, GameObject Target)
    {
       int debuffCounters = 2;

        tree.AddCurrent(AddDebuff(Attacker, Target, BuffName.ATTACK_DOWN, debuffCounters,tree));

        tree.CorQ.CoroutineCompleted();
        yield return null;
    }

    IEnumerator InflictAntiBuffCoroutine(GameObject Attacker, GameObject Target)
    {
       int debuffCounters = 2;

        tree.AddCurrent(AddDebuff(Attacker, Target, BuffName.ANTI_BUFF, debuffCounters,tree));
